...
**1. 메인 화면**
- **헤더**: 상단에 로고, 내 갤러리, 이미지 생성 메뉴를 포함한 헤더를 배치해 사용자가 주요 기능에 빠르게 접근할 수 있도록 합니다.
...
**1. 헤더 컴포넌트**
   - **레이아웃**: 화면 상단에 고정된 헤더로, 좌측에 로고, 중앙 또는 우측에 내비게이션 메뉴를 배치합니다.
   - **로고**: 좌측에 Artify 로고를 배치하여 브랜드 아이덴티티를 강조하고, 클릭 시 메인 화면으로 이동합니다.
   - **내비게이션 메뉴**: 
     - **내 갤러리**: 사용자가 저장한 이미지를 볼 수 있는 갤러리 페이지로 이동하는 링크입니다.
     - **이미지 생성**: 이미지 생성 페이지로 바로 이동할 수 있는 링크입니다.
   - **디자인 스타일**: 
     - 배경색은 흰색 또는 매우 연한 회색(#FAFAFA)을 사용하여 콘텐츠와 구분됩니다.
     - 메뉴 항목은 호버 시 메인 색상(#4A90E2)으로 하이라이트되며, 현재 페이지는 밑줄 또는 색상으로 표시됩니다.
     - 반응형 디자인을 적용하여 모바일에서는 햄버거 메뉴로 변환됩니다.
   - **상호작용**: 
     - 메뉴 항목에 호버 시 부드러운 색상 변화 애니메이션을 적용합니다.
     - 모바일 햄버거 메뉴는 클릭 시 부드럽게 슬라이드되어 나타납니다.
